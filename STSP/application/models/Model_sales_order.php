...
class Model_sales_order extends CI_Model{
	...
	function json() {
		...
		$this->datatables->add_column('view', '<a href="Sales_order/input_retur_noju/$1" class="btn btn-xs btn-primary"><i class = "fa fa-mail-forward"></i></a>', 'no_jual');
	...
	function insert_retur_noju(){
		...
		if($total==0){
			...
			redirect('Sales_order/input_retur_noju/'.$no_jual);
		}
		$data = array(
	...
	function insert_retur(){
		...
		$query = "delete from retur_tmp where user='$d->user'";
		$this->db->query($query);
		redirect('Sales_order/input_retur');
	...
	function getNomorterakhir1(){
		$query = "select no_sj from tb_penjualan ORDER BY no_sj DESC LIMIT 1 ";
		return $this->db->query($query);
	}
