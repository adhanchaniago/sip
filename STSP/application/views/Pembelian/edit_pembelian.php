 	...
 	<div class = "row " >
 		...
 		<section class="col-lg-12 connectedSortable">
 			<div class="box box-primary" > 
 				<h3 class="box-title" style="margin-top: 4px;margin-bottom: -19px;padding-bottom: 4PX;" >Edit Pembelian</h3>
 			</div>
 			<div class="box-body">
 				<div class="col-md-7">
 					<form class="form-horizontal"  method="POST" id="form" action="" enctype = "multipart/form-data">
 						<div class="form-group">
 							...
 							<div class="col-sm-37" style="margin-left: -5%;">
 								<input type="text" readonly  name="no_invoice" id="no_invoice" value="<?php echo $p['no_invoice'];?>"  class="form-control" placeholder = " No Invoice"  >
 							</div>
 					...
 					<div class="col-md-3" style="margin-top: -9px;">
 						...
 						<textarea style="display:none;"rows="1"class="form-control" name="kurs" 		  id="hasil_kurs_tukar1" style="height: 34px;"readonly requered></textarea>
 							...
 							<table class = "table1">
 								<tr>
 									<td align = "right">
 										...
 										<a href = "<?php echo base_url();?>Pembelian/riset_edit_pembelian" class = "btn btn-sm btn-danger"  tabindex="31">Cancel</a>
 ...
 <script>
 	...
 	$(function(){
 		...
 		$('#nama_barang').autocomplete({
 			serviceUrl: site+'Pembelian/get_data_edit/'+nobel,
 			onSelect: function (suggestion) {
 				...
 				$('#kus').val(''+suggestion.harga_beli1);
 				$('#kus').val(''+suggestion.harga_beli1);
 ...
 <script type="text/javascript">
 	...
 	function tampiltmp(){			
 		...
 		$.ajax({
 			type    : "POST",
 			url     : "<?php echo base_url(); ?>Pembelian/view_barang_edit/"+no_beli,
 		...
 		$.ajax({
 			type    : "POST",
 			url     : "<?php echo base_url(); ?>Pembelian/view_barang_edit3/"+no_beli,
 			data    : {no_beli:no_beli},
 			cache   : false,
 			success : function(respond){
 		...
 		$('.tampildata').load("<?php echo base_url();?>Pembelian/view_barang_edit2/"+no_beli);
 	}
           ...
           var modal_terakhir	=$('#modal_terakhir').val();
            ...
            if (pl1 > 0) {
            	if (satu1 <= mod ) {
            		...
            		$.ajax({
            			type : "POST",
            			url  : "<?php echo base_url()?>Pembelian/input_barang_edit",
                data : {idbarang:idbarang,nobel:nobel,modalt:modalt,stok:stok,qtybes:qtybes,satuanbes:satuanbes,modal_terakhir:modal_terakhir,hargabeli:hargabeli,modal:modal,ppn:ppn,disc:disc,disc1:disc1,disc2:disc2,jam:jam,pl:pl,hajul:hajul,walkk:walkk,tkw:tkw,tbn:tbn,slss:slss}, //dihapus,disc2:disc2
	...
	$.ajax({
		type : "POST",
		url  : "<?php echo base_url()?>Pembelian/input_barang_edit",
                data : {idbarang:idbarang,nobel:nobel,modalt:modalt,stok:stok,qtybes:qtybes,satuanbes:satuanbes,modal_terakhir:modal_terakhir,hargabeli:hargabeli,modal:modal,ppn:ppn,disc:disc,disc1:disc1,disc2:disc2,jam:jam}, //dihapus,disc2:disc2
                success: function(data){
                	$('[name="idbarang"]').val("");
                	$('[name="namabarang"]').val("");
                	$('[name="qtybes"]').val("");
...
<script>
	...
	$('body').on('click', '.hapus-barang', function(e){
		...
		$.ajax({
			type: 'post',
			dataType: 'json',
			url : "<?= site_url('Pembelian/destroy_edit/');?>"+'/'+user+'/'+id_barang,
			success: function(data){
				tampiltmp();
				$('.tampildata1').load("<?php echo base_url();?>Pembelian/view_barang_edit3");
			}
