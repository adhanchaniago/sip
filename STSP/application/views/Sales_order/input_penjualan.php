...
<div class = "row " >
	<section class="col-lg-12 ">
		<div class="box box-primary" > 
			<h3 class="box-title">Input Sales Order</h3>
		</div>
		<div class="box-body">
			<form class="form-horizontal"  method="POST" id="form" action="" enctype = "multipart/form-data">
				<div class="form-group">
					...
					<div class="col-sm-37">
						...
						<textarea rows="1" style="display:none;" class="form-control" name="jam" id="txt" readonly></textarea>
					...
					<div class="col-sm-37">
						<select name="id_plg" id="id_pelanggan" class="form-control select2 id_pelanggan"  autofocus tabindex="1" required style="width: 100%"> 
					...
					<textarea  type="text"  name="ket_alamat"  maxlength="50"  id="send" rows="2" autocomplete="off" class="form-control"  placeholder = " SHIP TO (F1) " onkeyup=" this.value=this.value.toUpperCase();"></textarea>
					...
					<div class="col-sm-35" >
						<input type="text"  name="qtybes" id="Q_B" autocomplete="off" onFocus="startCalc();" class="form-control" placeholder = "QTY "  tabindex = "6">
				...
				<div class="form-group">
					...
					<?php endif;?>
					<a href = "<?php echo base_url();?>Sales_order/riset" class = "btn btn-sm btn-danger"  tabindex="31">Cancel</a>
				</div>
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form-group">
					<label class="col-sm-40 control-label" >password</label>
					<div class="col-sm-37" >
...
<script>
	...
	function cekbarang(){
		...
		$.ajax({
			type    : 'POST',
			url     : '<?php echo site_url(); ?>Sales_order/cekbarang',
			cache   : false,
	...
	$(function(){
		$('#nama_barang').autocomplete({
			serviceUrl: site+'Sales_order/get_data',
...
<script type="text/javascript">
	...
	function tampilan(){			
		...
		$("#tampilan").load("<?php echo base_url();?>Sales_order/view_detail_retur");	
	}
	$('#bt_simpan').on('click',function(){
		...
		if(nmbarang == 0){
			...
			$.ajax({
				type : "POST",
				url  : "<?php echo base_url()?>Sales_order/input_detail_retur",
...
<script type="text/javascript">
	...
	function tampiltmp(){			
		...
		$("#tampiltmp").load("<?php echo base_url();?>Sales_order/view_detail_barang");
		$('.tampildata1').load("<?php echo base_url();?>Sales_order/view_detail_barang3");	
		...
		if(namabarang == 0){
			...
			$('.ok').click(function () {
				...
				if (password != confirmPassword) {
					...
					$.ajax({
						type : "POST",
						url  : "<?php echo base_url();?>Sales_order/input_detail_barang",
						data : {
							idbarang 	: idbarang,
							qtybes 		: qtybes,
							satuanbes 	: satuanbes,
...
<script>
	...
	$('body').on('click', '.hapus_barang', function(e){
		...
		$.ajax({
			type: 'post',
			dataType: 'json',
			url : "<?= site_url('Sales_order/destroy/');?>"+'/'+user+'/'+id_barang,
			success: function(data){
				cekbarang();
				tampiltmp();
				$('.tampildata').load("<?php echo base_url();?>Sales_order/view_detail_barang2");
