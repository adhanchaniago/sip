...
<form class="form-horizontal"  method="POST" action="<?php echo base_url();?>Surat_jalan/update_cetak_sj" target = "" enctype = "multipart/form-data">
							<div class="form-group">
								<label class="col-sm-3 control-label">Alasan Print</label>
								<div class="col-sm-5">
										<?php foreach ($cetak->result() as $r) {  ?>
										<input type="hidden" value = "<?php echo $r->no_so; ?>" name="no_sj" id="#" autocomplete="off" class="form-control" placeholder = "Masukan Alasan">
										<input type="hidden" value = "<?php echo $r->no_kirim; ?>" name="no_kirim" id="#" autocomplete="off" class="form-control" placeholder = "Masukan Alasan">
										<?php } ?>
