...
<form class="form-horizontal"  method="POST" action="<?php echo base_url();?>Surat_jalan/update_cetak_sj" target = "tampil" enctype = "multipart/form-data">
	<div class="form-group">
		<label class="col-sm-3 control-label">Alasan Print</label>
		<div class="col-sm-5">
			<?php foreach ($cetak->result() as $r) {  ?>
				<input type="hidden" value = "<?php echo $r->no_sj; ?>" name="no_sj" id="#" autocomplete="off" class="form-control" placeholder = "Masukan Alasan">
			<?php } ?>
			...
			<table align = "right">
				<tr>
					<td align = "right">
						<?php if($this->session->userdata('level') === 'Administrator' OR $this->session->userdata('level') === 'KasirA5' OR $this->session->userdata('level') === 'Manager'): ?>
						<input type="submit" name = "submit" value = "CETAK A5" class="btn btn-sm btn-info">
					<?php endif;?>
				</td>
			</tr>
		</table>
