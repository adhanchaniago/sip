...
<form class="form-horizontal"  method="POST" id="form" action="<?php echo base_url();?>Penjualans/update_cetak1" target = "" enctype = "multipart/form-data">
	<div class="form-group">
		<label class="col-sm-3 control-label">Alasan Print</label>
		<div class="col-sm-5">
			<?php foreach ($cetak->result() as $r) {  ?>
				<input type="text" value = "<?php echo $r->no_jual; ?>" name="no_jual" id="#" autocomplete="off" class="form-control" placeholder = "Masukan Alasan">
			<?php } ?>
			...
			<table align = "right">
				<tr>
					<td align = "right">
						<?php if($this->session->userdata('level') === 'Administrator' OR $this->session->userdata('level') === 'KasirA5' OR $this->session->userdata('level') === 'Manager'): ?>										
						<input type="submit" name = "submit2" id="submit2" value = "CETAK A5" class="btn btn-sm btn-success">
					<?php endif;?>
				</td>
			</tr>
		</table>
